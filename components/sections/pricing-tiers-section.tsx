      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        {section.items.map((item) => (
          ...
          >
            ...
            <p className="mt-2 text-2xl font-semibold tracking-tight text-text-primary">{item.price}</p>
            ...
            <ul className="mt-5 space-y-2">
              {item.bullets.map((bullet) => (
                <li key={bullet} className="flex items-start gap-2 text-sm leading-relaxed text-text-secondary">
                  <span className="mt-1 block size-1.5 rounded-full bg-primary/90" aria-hidden="true" />
                  <span>{bullet}</span>
                </li>
              ))}
            </ul>
